...
int main(int argc, char **argv) {
  ...
  camera.start();
  std::cout << "[Run] Format: "
            << (fmt == CameraFormat::YUYV ? "YUYV" : "MJPEG")
  ...
  // ==================== 2. 实时主循环 ====================
  TIMER_TEST_BEGIN(main_test);

  while (display.running()) {
    auto t0 = std::chrono::steady_clock::now();

    // Step 1: 取帧
    TIMER_STEP_START(main_test, grab);
    if (!camera.grab())
      continue;
    TIMER_STEP_END(main_test, grab);

    // Step 2: RGA 硬件处理
    TIMER_STEP_START(main_test, rga);
    rga.convert_gray(camera.src_ptr(), camera.src_stride(), camera.src_w(),
                     camera.src_h(), camera.src_fmt());
    rga.convert_full_bgr(camera.src_ptr(), camera.src_stride(), camera.src_w(),
                         camera.src_h(), camera.src_fmt());
    TIMER_STEP_END(main_test, rga);

    // Step 3: ROI 预测（直接使用 FusionROIPredictor）
    TIMER_STEP_START(main_test, roi);
    ...
    TIMER_STEP_START(main_test, det);
    for (size_t i = 0; i < rois.size() && i < 7; ++i) {
      if (!rga.crop_to_ptr(camera.src_ptr(), camera.src_stride(),
                           camera.src_w(), camera.src_h(), camera.src_fmt(),
    ...
    TIMER_TEST_PEEK(main_test);
  }
  TIMER_TEST_END(main_test);
