...
static void print_usage(const char *prog) {
  std::cout
      ...
      << "  --save-det <n>         Save detection images every N frames "
         "(0=off)\n"
...
cross_region_nms(const std::vector<std::vector<Detection>> &all_tile_dets,
...
int main(int argc, char **argv) {
  ...
  int c, opt_idx = 0;
  while ((c = getopt_long(argc, argv, "d:W:H:f:m:c:i:b:l:w:t:u:C:R:x:y:o:sS:h",
                          long_opts, &opt_idx)) != -1) {
    switch (c) {
    case 'd':
      dev = optarg;
  ...
  void *dst_ptr = mmap(nullptr, cropper.dst_size(), PROT_READ, MAP_SHARED,
                       cropper.dst_fd(), 0);
  ...
  if (det.input_width() != tile_w || det.input_height() != tile_h) {
    std::cerr << "[Test] ERROR: Model input size " << det.input_width() << "x"
              << det.input_height() << " does not match tile size " << tile_w
              << "x" << tile_h << "\n";
    return 1;
  }
  ...
  while (g_running) {
    ...
    bool grab_success = cam.grab();
    TIMER_STEP_END_IF(pipeline, grab, grab_success && (frame_count > warmup));
    if (!grab_success) {
      continue;
    }
    ...
    TIMER_STEP_END_IF(pipeline, rga_submit, frame_count > warmup);
    auto t2 = std::chrono::steady_clock::now();

    // RGA job 已提交，归还 V4L2 buffer
    bool save_this_frame =
        (save_det_interval > 0 && frame_count >= warmup &&
    ...
    if (save_this_frame) {
      ...
      if (yuyv_raw) {
        cv::Mat yuyv(cam.src_h(), cam.src_w(), CV_8UC2, yuyv_raw,
                     cam.src_stride());
        cv::cvtColor(yuyv, save_bgr, cv::COLOR_YUV2BGR_YUYV);
      } else if (cam.src_fd() >= 0) {
        yuyv_mmap_sz = (size_t)cam.src_stride() * cam.src_h();
        yuyv_mmap = (uint8_t *)mmap(nullptr, yuyv_mmap_sz, PROT_READ,
                                    MAP_SHARED, cam.src_fd(), 0);
        if (yuyv_mmap != MAP_FAILED) {
          cv::Mat yuyv(cam.src_h(), cam.src_w(), CV_8UC2, yuyv_mmap,
                       cam.src_stride());
          cv::cvtColor(yuyv, save_bgr, cv::COLOR_YUV2BGR_YUYV);
        }
      }
      if (yuyv_mmap && yuyv_mmap != MAP_FAILED)
        munmap(yuyv_mmap, yuyv_mmap_sz);
    }
    ...
    TIMER_STEP_END_IF(pipeline, rga_wait, frame_count > warmup);
    ...
    TIMER_STEP_END_IF(pipeline, infer, frame_count > warmup);
    ...
    auto merged_dets = cross_region_nms(all_dets, tile_cols, tile_rows, tile_w,
                                        tile_h, overlap_w, overlap_h, iou);
    TIMER_STEP_END_IF(pipeline, cross_nms, frame_count > warmup);

    TIMER_TEST_PEEK_RECENT_EVERY_T(pipeline, 1000);
    ...
    if (save_this_frame && !save_bgr.empty() && !merged_dets.empty()) {
      for (const auto &d : merged_dets) {
        ...
        cv::Rect bg((int)d.x, (int)d.y - ts.height - 4, ts.width + 8,
                    ts.height + 4);
      ...
      std::string timing =
          cv::format("grab:%.1f rga:%.1f copy:%.1f infer:%.1f total:%.1f ms",
                     s.grab_ms, s.rga_ms, s.copy_ms, s.infer_ms, s.total_ms);
      int baseline = 0;
      cv::Size ts =
          cv::getTextSize(timing, cv::FONT_HERSHEY_SIMPLEX, 0.45, 1, &baseline);
      cv::rectangle(save_bgr, cv::Point(0, 0),
                    cv::Point(ts.width + 8, ts.height + 8), cv::Scalar(0, 0, 0),
                    -1);
      cv::putText(save_bgr, timing, cv::Point(4, ts.height + 4),
                  cv::FONT_HERSHEY_SIMPLEX, 0.45, cv::Scalar(0, 255, 0), 1,
                  cv::LINE_AA);
      std::string save_path =
          output_dir + "/det_" + std::to_string(frame_count - warmup) + ".jpg";
    ...
    if (save_tiles && frame_count == warmup && !saved) {
      ...
      if (RgaCropper::save_tiles(cropper.dst_fd(), cropper.dst_size(), tile_w,
                                 tile_h, cropper.tile_count(), prefix)) {
